class GoogleCalendarAppointment
  ...
  def client_appointment
    Google::Apis::CalendarV3::Event.new({
      summary: "New Appointment for #{appointment.stylist.first_name} Client:#{appointment.name}",
      location: 'Karma Salon',
      description: "Treatment Services #{apt.treatment_services || 'Not requested'}, Chemical Services #{apt.chemical_services || 'Not requested'},Color services #{apt.color_services || 'Non requested'}, Braid #{apt.braids || 'Not requested'}, Basic services #{apt.basic_services || 'Not requested'}, Weave #{apt.weave || 'Not requested'}",
      start: {
        date_time: "#{appointment.start_time.strftime('%Y-%m-%dT%H:%M:%S')}",
        time_zone: 'America/New_York',
      },
      end: {
        date_time: "#{(appointment.start_time + 1.hour).strftime('%Y-%m-%dT%H:%M:%S')}",
        time_zone: 'America/New_York',
      }
    })
  end
